...
const clearModule = clear;
import { add, push } from "../lib/git.js";
import {
  gitCommitedFiles,
  gitCommits,
  gitDetachedHead,
...
test("Verify authentication only on the fist call", async (t) => {
  ...
  t.notThrows(() =>
    t.context.m.verifyConditions({}, { cwd, options, logger: t.context.logger })
  );
...
test("Throw SemanticReleaseError if config is invalid", (t) => {
  ...
  const errors = [
    ...t.throws(() =>
      t.context.m.verifyConditions({ message, assets }, {
        options: {},
        logger: t.context.logger,
      })
    ),
